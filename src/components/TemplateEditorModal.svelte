<script>
  ...
  function checkDeviceCooldown() {
    try {
      const lastTime = parseInt(
        localStorage.getItem("neocgpa_last_pub_time") || "0",
        10,
      );
  ...
  $effect(() => {
    if (isOpen && !wasOpen) {
      ...
      if (initialTemplate) {
        // Load initial template into isolated editor state & lock ID
        ...
        isIdLocked = true; // Lock ID for existing published templates!

        // Clone semesters and courses deeply so edits don't mutate original objects
        if (
          Array.isArray(initialTemplate.semesters) &&
          initialTemplate.semesters.length > 0
        ) {
  ...
  async function handleDeleteCloudTemplate() {
    if (!docIdSuffix || !docIdSuffix.trim()) return;
    if (!passcode || !passcode.trim()) {
      publishError = "Secret Passcode is required to delete this template.";
      return;
    }

    const fullDocId = `TMP-${docIdSuffix.trim()}`;
    isDeleting = true;
    publishError = "";
    ...
    try {
      ...
      alert(`🗑️ Template "${fullDocId}" has been deleted from cloud.`);
      onClose();
    } catch (err) {
      publishError = err.message || "Failed to delete cloud template.";
    } finally {
      isDeleting = false;
  ...
  function handleRemoveCourse(semIndex, courseIndex) {
    semesters[semIndex].courses = semesters[semIndex].courses.filter(
      (_, cI) => cI !== courseIndex,
    );
  }
  ...
  async function handlePublishToCloud() {
    ...
    try {
      const metadata = {
        name:
          [institution, branch, year].filter(Boolean).join(", ") || fullDocId,
        institution,
        branch,
        year,
      };
  ...
  function handleApplyToWorkspace() {
    const name =
      [institution, branch, year].filter(Boolean).join(", ") || "Custom Scheme";
...
{#if isOpen}
  ...
  >
    ...
    >
      <!-- Modal Header -->
      <div
        class="bg-[#FF70A6] border-b-3 border-black p-4 flex items-center justify-between text-white"
      >
        <div class="flex items-center gap-3">
          <div
            class="w-9 h-9 bg-black text-[#86EFAC] border-2 border-black flex items-center justify-center font-bold text-lg shadow-[2px_2px_0px_0px_#000]"
          >
            <Edit3 class="w-5 h-5" />
          </div>
          <div>
            <div class="flex items-center gap-2">
              <h3 class="font-display font-black text-xl text-black">
                Template Scheme Editor Studio
              </h3>
              <span
                class="bg-black text-[#86EFAC] text-[10px] font-mono font-bold px-2 py-0.5 border border-black shadow-[1px_1px_0px_0px_#000]"
              >
                ISOLATED MODE — Personal Workspace Untouched
              </span>
            </div>
            <p class="text-xs font-mono text-zinc-900 font-bold">
              Edit university courses, subjects, and credits directly without
              modifying your active GPA calculations.
            </p>
          </div>
        </div>
      ...
      <div class="p-6 overflow-y-auto flex-1 space-y-6">
        ...
        {#if publishSuccess}
          <div
            class="neo-box bg-[#DCFCE7] p-4 border-2 border-black space-y-3 animate-in fade-in zoom-in-95"
          >
            <div class="flex items-center justify-between">
              <div
                class="flex items-center gap-2 font-mono text-xs font-black text-[#166534]"
              >
                <Sparkles class="w-4 h-4 text-green-700" />
                <span
                  >{publishSuccess.isUpdate
                    ? "✅ Cloud Scheme Updated Successfully!"
                    : "🎉 Published to Cloud Successfully!"}</span
                >
              </div>
              <span
                class="font-mono text-[10px] font-bold bg-white border border-black px-1.5 py-0.5"
                >ID: {publishSuccess.docId}</span
              >
            </div>

            <div>
              <label
                for="editor-share-url-input"
                class="block text-[11px] font-mono font-bold text-black mb-1"
                >Direct Teammate Share Link:</label
              >
              <div class="flex items-center gap-2">
                <input
        ...
        <div class="neo-box bg-white p-5 space-y-4 border-2">
          <h4
            class="font-display font-black text-base text-black flex items-center gap-2 border-b-2 border-black/10 pb-2"
          >
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <div class="flex items-center justify-between mb-1">
                <label
                  for="editor-id-suffix"
                  class="text-xs font-mono font-bold text-black"
                  >Template ID * (Prefix: TMP-)</label
                >
                {#if isIdLocked}
                  <span
                    class="text-[10px] text-amber-950 font-mono font-bold bg-amber-200 border border-black px-1"
                    >🔒 ID Locked (Fixed for Update)</span
                  >
                {:else if idCheckStatus === "idle"}
                  <span class="text-[10px] text-zinc-500 font-mono"
                    >Enter 3-8 chars</span
                  >
                ...
                {:else if idCheckStatus === "available"}
                  <span
                    class="text-[10px] text-green-700 font-mono font-bold bg-green-100 border border-green-800 px-1"
                    >✅ Available ID</span
                  >
                {:else if idCheckStatus === "taken"}
                  <span
                    class="text-[10px] text-amber-900 font-mono font-bold bg-amber-100 border border-amber-800 px-1"
                    >⚠️ ID Taken (Passcode Required)</span
                  >
                {:else if idCheckStatus === "invalid"}
                  <span class="text-[10px] text-red-600 font-mono font-bold"
                    >Min 3 chars</span
                  >
                {/if}
              </div>
              ...
              <div class="flex items-center">
                <span
                  class="bg-black text-[#FFDE59] text-xs font-mono font-black px-3 py-2 border-2 border-black border-r-0 shadow-[1.5px_1.5px_0px_0px_#000] shrink-0"
                >
                  TMP-
                </span>
                <input
                  ...
                  maxlength="8"
                  class="neo-input text-xs font-mono font-black py-2 uppercase {isIdLocked
                    ? 'bg-zinc-200 text-zinc-800 opacity-90 cursor-not-allowed border-2 border-black'
                    : ''}"
                />
              </div>
            </div>
            ...
            <div>
              <label
                for="editor-passcode"
                class="block text-xs font-mono font-bold text-black mb-1 flex items-center gap-1"
              >
            ...
            <div>
              <label
                for="editor-inst"
                class="block text-xs font-mono font-bold text-black mb-1"
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            <h4
              class="font-display font-black text-lg text-black flex items-center gap-2"
            >
              <BookOpen class="w-5 h-5 text-[#FF70A6]" />
              <span
                >Semesters & Course Subjects ({semesters.length} Semesters)</span
              >
            </h4>
          ...
          <div class="space-y-4">
            {#each semesters as sem, semIdx (sem.id || semIdx)}
              <div class="neo-box bg-white p-4 space-y-3 border-2">
                <!-- Semester Header Row -->
                <div
                  class="flex items-center justify-between gap-3 border-b-2 border-black/10 pb-2"
                >
                  <div class="flex items-center gap-2 flex-1">
                    <span
                      class="w-3 h-3 bg-[#FF70A6] border border-black shrink-0"
                    ></span>
                    <input
                      type="text"
                      bind:value={sem.name}
                    ...
                    <span class="text-xs font-mono font-bold text-zinc-500"
                      >({sem.courses.length} subjects)</span
                    >
                  </div>
                ...
                <div class="overflow-x-auto">
                  <table class="w-full text-left text-xs font-mono">
                    <thead>
                      <tr
                        class="bg-[#FAF7EE] border-b-2 border-black text-black"
                      >
        ...
        <div class="neo-box bg-[#FFFDF5] p-4 space-y-4 border-2">
          ...
          <div
            class="neo-box bg-[#FAF7EE] p-3.5 border-2 border-black space-y-2"
          >
            <div class="flex items-center justify-between gap-2">
              <label
                for="editor-captcha-input"
                class="text-xs font-mono font-bold text-black flex items-center gap-1.5"
              >
            ...
            <div class="flex items-center gap-3">
              <div
                class="bg-[#FFDE59] border-2 border-black px-3 py-1.5 font-mono text-base font-black tracking-widest text-black shadow-[2px_2px_0px_0px_#000] select-none italic underline decoration-wavy decoration-black/40 shrink-0"
              >
                {captchaCode}
              </div>
          ...
          {#if publishError}
            <div
              class="neo-box bg-[#FFD1D1] border-2 border-black p-3 text-xs font-mono font-black text-black flex items-center gap-2 shadow-[2px_2px_0px_0px_#000]"
            >
              <AlertTriangle
                class="w-4 h-4 text-red-700 shrink-0 stroke-[2.5]"
              />
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <button
              ...
              disabled={isPublishing ||
                !isCaptchaVerified ||
                (!isDirty && !isJustUploaded) ||
                (cooldownRemaining > 0 && !isJustUploaded)}
              class="neo-btn py-3 text-xs font-black flex items-center justify-center gap-2 shadow-[2px_2px_0px_0px_#000] transition-colors {isJustUploaded
                ? 'bg-[#4ADE80] text-black border-2 border-black font-black'
                : cooldownRemaining > 0
                  ? 'bg-zinc-200 text-black border-2 border-black font-black opacity-100'
                  : !isDirty
                    ? 'bg-zinc-100 text-zinc-800 opacity-70 border-2 border-black'
            ...
            >
              ...
              <span class="text-black font-black">
                ...
                {:else if idCheckStatus === "taken"}
                  Update Cloud Template (TMP-{docIdSuffix})
                {:else}
            ...
            {#if isIdLocked}
              <button
                ...
                disabled={isDeleting}
                class="neo-btn bg-[#FF4757] hover:bg-red-700 text-white py-2.5 text-xs font-black flex items-center justify-center gap-2 shadow-[2px_2px_0px_0px_#000] sm:col-span-2 mt-1"
              >
                <Trash2 class="w-4 h-4 stroke-[2.5]" />
                <span
                  >{isDeleting
                    ? "Deleting..."
                    : `Delete Cloud Template (TMP-${docIdSuffix})`}</span
                >
              </button>
            {/if}
          </div>
      ...
      <!-- Modal Footer -->
      <div
        class="bg-[#FAF7EE] border-t-3 border-black p-4 flex items-center justify-between"
      >
        <span class="text-xs font-mono font-bold text-zinc-600">
          Editing this scheme in studio mode does not alter your active GPA
          calculation.
        </span>
...
<!-- Confirmation Modal for Deleting Cloud Template -->
<ConfirmModal
  isOpen={isDeleteConfirmOpen}
  title="Delete Cloud Template TMP-{docIdSuffix}?"
  message="Are you sure you want to permanently delete cloud template TMP-{docIdSuffix} from Firestore? This action cannot be undone."
  type="danger"
  confirmText="Delete Template"
  onConfirm={handleDeleteCloudTemplate}
